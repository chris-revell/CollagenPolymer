...
@inline function initialise(pos::MMatrix,Nmonomers::Int64,Ndomains::Int64,domainLength::Float64,boxsize::Float64)
    ...
    for ii=1:Nmonomers
        ...
        for jj=1:Ndomains
            pos[(ii-1)*Ndomains+jj,1] = initialx[1]+(jj-1)*domainLength*cos(initialAngle[1])*sin(initialAngle[2]);
            pos[(ii-1)*Ndomains+jj,2] = initialx[2]+(jj-1)*domainLength*sin(initialAngle[1])*sin(initialAngle[2]);
            pos[(ii-1)*Ndomains+jj,3] = initialx[3]+(jj-1)*domainLength*cos(initialAngle[2]);
        end
    end
end
